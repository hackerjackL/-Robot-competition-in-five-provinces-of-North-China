...
def getAreaMaxContour(contours):
    ...
    for c in contours:  # 历遍所有轮廓
        ...
        if contour_area_temp > contour_area_max:
            ...
            if contour_area_temp >= 100:  # 只有在面积大于100时，最大面积的轮廓才是有效的，以过滤干扰
                area_max_contour = c
    ...
    while True:
        if distance != 99999:
            if j == 0 and distance <= 350:    # 如果距离小于350，则向右转向
                ...
                for i in range(3):
                    AGC.runActionGroup("turn_right_fast")   # 向右转向
                AGC.runActionGroup("turn_left_fast")     # 向左转向    这个是个矫正，csdn有写，左右来回矫正防止找不到线
                j = j + 1
            elif j == 1 and distance <= 400:        # 如果距离小于400，则向左转向
                for i in range(8):
                    AGC.runActionGroup("left11")      # 向左转向
                for i in range(1):
                    AGC.runActionGroup("turn_left_fast")      # 向左转向
                    AGC.runActionGroup("turn_right_fast")            # 向右转向
                j = j + 1
            elif j == 2 and distance <= 300:
                for i in range(9):
                    AGC.runActionGroup("right_move_fast")      # 向右移动
                for i in range(2):
                    AGC.runActionGroup("turn_right_fast")        # 向右转向
            else:
                if __isRunning:
