...
def load_all_area_data(data_dir='area_test'):
    """
    加载指定目录下所有2025版本面积数据JSON文件
    """
    json_files = glob.glob(os.path.join(data_dir, '2025_area_data_*.json'))
...
def save_statistics(statistics, output_file='area_test/2025_area_statistics.json'):
    """
    保存统计结果到JSON文件
    """
    # 确保输出目录存在
...
if __name__ == '__main__':
    # 加载所有面积数据
    print("正在加载2025版本面积数据文件...")
    all_data = load_all_area_data('area_test')
    
    if not all_data:
        print("未找到任何2025版本面积数据文件！")
        print("请先运行 2025_area_test_detection.py 生成数据文件。")
        exit(1)
